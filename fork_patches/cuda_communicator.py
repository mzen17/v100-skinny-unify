...
# Changes: routes small fp16 TP=2 all-reduces through kernels/skinny_ar.cu,
# a host-memory all-reduce for boxes without GPU P2P (VLLM_SKINNY_AR=0 to
# disable). Measured 27 us vs NCCL LL 44 us per 40 KB decode all-reduce.
...
class CudaCommunicator(DeviceCommunicatorBase):
    def __init__(
        ...
        # skinny_ar: 2-rank host-memory all-reduce for small fp16 messages.
        self._skinny_ar = None
        self._skinny_ar_max = 0
        if (
            self.world_size == 2
            and "tp" in unique_name
            and os.environ.get("VLLM_SKINNY_AR", "1") == "1"
        ):
            try:
                self._skinny_ar_init(unique_name)
            except Exception as exc:  # never fail a boot on this
                logger.warning("skinny_ar disabled: %s", exc)
                self._skinny_ar = None
    ...
    def _skinny_ar_init(self, unique_name: str) -> None:
        import re

        from torch.utils.cpp_extension import load

        src = os.environ.get("VLLM_SKINNY_AR_SRC")
        if not src:
            nv = os.environ.get("VLLM_SKINNY_NVFP4_SRC")
            if nv:
                src = os.path.join(os.path.dirname(nv), "skinny_ar.cu")
        if not src or not os.path.exists(src):
            raise RuntimeError("VLLM_SKINNY_AR_SRC not set / not found")
        ext = load(
            name="skinny_ar_v1",
            sources=[src],
        ...
        max_bytes = int(os.environ.get("VLLM_SKINNY_AR_MAX_BYTES", str(256 * 1024)))
        tag = re.sub(r"[^A-Za-z0-9_]", "_", unique_name)
        path = f"/dev/shm/skinny_ar_{tag}_{os.getppid()}"
        rank = torch.distributed.get_rank(group=self.cpu_group)
        with torch.cuda.device(self.device):
            ext.init(path, rank, 2, max_bytes)
        torch.distributed.barrier(group=self.cpu_group)
        # Both ranks hold the mapping now; unlink so /dev/shm does not
        # accumulate one file per boot (the mapping outlives the name).
        if rank == 0:
            try:
                os.unlink(path)
            except OSError:
                pass
        self._skinny_ar = ext
        self._skinny_ar_max = max_bytes
        logger.info(
            "skinny_ar: host-memory 2-rank all-reduce active for fp16 messages "
            "<= %d bytes (shm %s)",
            max_bytes,
            path,
        )
    ...
    def all_reduce(self, input_):
        sk = self._skinny_ar
        if sk is not None and input_.dtype == torch.float16 and input_.is_contiguous():
            nbytes = input_.numel() * 2
            if (
                0 < nbytes <= self._skinny_ar_max
                and nbytes % 16 == 0
                and input_.data_ptr() % 16 == 0
            ):
                out = sk.all_reduce(input_, 1)
                _trace_all_reduce_path(self, "skinny_ar", input_)
                return out
        return self._all_reduce_stock(input_)

    def _all_reduce_stock(self, input_):
    ...
    def sm70_tp2_all_reduce_gemma_rms_norm(
        ...
        if (
            ...
            and 1 <= input_.shape[0] <= 25
            and input_.shape[1] == 5120
            and input_.numel() * 2 <= self._skinny_ar_max
            and input_.data_ptr() % 16 == 0
            and residual.shape == input_.shape
            and residual.is_contiguous()
            and residual.dtype in (torch.float16, torch.float32)
            and weight.is_contiguous()
            and weight.dtype in (torch.float16, torch.float32)
        ):
            out, res_out = sk.all_reduce_gemma_norm(
                input_, residual, weight, float(epsilon)
            )
            _trace_all_reduce_path(self, "skinny_ar_gemma_norm", input_)
            return out, res_out
        ca_comm = self.ca_comm
        if (
