...
# Changes: the SM70 TP2 all-reduce + Gemma RMSNorm fusion also activates when
# the communicator carries skinny_ar (host-memory all-reduce), not only the
# P2P custom all-reduce; and a pattern variant with GemmaRMSNorm's redundant
# fp32 re-cast, which is what the post-grad graph actually contains.
...
class Sm70AllReduceGemmaRMSNormPattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        ...
        # v100-skinny: GemmaRMSNorm._forward_static_with_residual re-casts the
        # fp32 sum (`x = x.float()`) before the variance, and that redundant
        # convert survives to the post-grad graph, so the pattern above never
        # matched (0 replacements). Register the same pattern with the cast.
        def pattern_cast(
            residual: torch.Tensor,
            input: torch.Tensor,
            weight: torch.Tensor,
            ...
            reduced = tensor_model_parallel_all_reduce(input)
            residual_out = reduced.float() + residual.float()
            x = torch.ops.prims.convert_element_type.default(residual_out, torch.float32)
            variance = x.pow(2).mean(dim=-1, keepdim=True)
            normalized = x * torch.rsqrt(variance + self.epsilon)
            normalized = normalized * (1.0 + weight.float())
            return normalized.to(input.dtype), residual_out

        # fwd_only's functional passes would strip the same-dtype cast from
        # the pattern (making it a duplicate of the one above); trace without
        # them so the pattern keeps the convert the graph has.
        import functools

        trace_keep_casts = functools.partial(pm.fwd_only, run_functional_passes=False)
        pm.register_replacement(
            pattern_cast,
            replacement,
            self.get_inputs(),
        ...
        pm.register_replacement(
            first_return_only(pattern_cast),  # type: ignore[no-untyped-call]
            first_return_only(replacement),  # type: ignore[no-untyped-call]
            self.get_inputs(),
            trace_keep_casts,
            pm_pass,
...
class AllReduceFusionPass(VllmPatternMatcherPass):
    def __init__(self, config: VllmConfig) -> None:
        ...
        if self.sm70_mode:
            ...
            skinny_ar = getattr(device_comm, "_skinny_ar", None)
            if (
                not isinstance(ca_comm, CustomAllreduce) or ca_comm.disabled
            ) and skinny_ar is None:
                logger.warning_once(
