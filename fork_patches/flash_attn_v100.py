...
class FlashAttnV100Impl(TritonAttentionImpl):
    ...
    def _smallq_decode_xqa_allowed(
        ...
        *,
        ...
        # e4m3 reaches the same kernel now that flash_attn_v100 has a packed
        # E4M3 decoder. This is the gate that matters for long context: it is
        # the small-q (speculative verify) decode path and only engages above
        # SMALLQ_DECODE_XQA_MIN_SEQ_LEN, which is exactly where the e4m3-vs-e5m2
        # gap appeared.
        fp8_e4m3_kv = (
        ...
        if not (fp16_kv or fp8_e5m2_kv or fp8_e4m3_kv):
            return False
    ...
    def _flash_v100_decode(
        ...
        xqa_kv_supported = (
            ...
            # e4m3 XQA: enabled by the patched flash_attn_v100 kernel, which
            # decodes E4M3 with a packed shift and folds the resulting 2^-8
            # into k_scale/v_scale. Unlike e5m2 this keeps the checkpoint's
            # calibrated KV scales, so there is no quality cost.
            self.kv_cache_dtype in ("fp8", "fp8_e4m3")
            and key_cache.dtype == torch.uint8
            and value_cache.dtype == torch.uint8
        ) or (
    ...
    def _should_use_fp8_prefill_bridge(
        ...
        *,
        ...
        return (
            self.use_fp8_prefill_bridge
            and self.use_flash_v100_prefill_paged
            and self.kv_cache_dtype in ("fp8_e5m2", "fp8", "fp8_e4m3")
            and key_cache.dtype == torch.uint8
            and value_cache.dtype == torch.uint8
    ...
    def _run_fp8_prefill_bridge(
        ...
        *,
        ...
        self.fp8_e5m2_paged_kv_to_fp16(
            ...
            k_scale,
            v_scale,
            is_e4m3=self.kv_cache_dtype in ("fp8", "fp8_e4m3"),
        )
