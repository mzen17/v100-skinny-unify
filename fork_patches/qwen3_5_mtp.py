# Modified by v100-skinny (2026-09-04), Apache-2.0: pipeline-parallel support
# for the MTP drafter -- `SupportsPP` marker, always-embed predictor forward
# (the MTP head is never pipeline-split), and a drafter-owned embedding when
# PP>1 (the target embedding lives on the first stage). Original file:
# vllm/model_executor/models/qwen3_5_mtp.py from 1Cat-vLLM 1.2.2.
...
class Qwen3_5MultiTokenPredictor(nn.Module):
    ...
    def forward(
        ...
        # The MTP head is never pipeline-split: it runs whole on the last
        # stage, so it always embeds its own inputs (PP-safe).
        if True:
...
class Qwen3_5MTP(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        super().__init__()
        self.config = config
        # Under pipeline parallelism the drafter lives on the LAST stage while
        # the target's embed_tokens lives on the FIRST, so the drafter must own
        # an embedding copy; lm_head is on the last stage and stays shared.
        pp_size = get_pp_group().world_size
        self.model = Qwen3_5MultiTokenPredictor(
            vllm_config=mtp_vllm_config,
            prefix=maybe_prefix(prefix, "mtp"),
            share_target_embed_tokens=(self.share_target_io_weights and pp_size == 1),
        )
