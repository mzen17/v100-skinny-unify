# Modified by v100-skinny (2026-09-04), Apache-2.0: routes the GDN in_proj_ba
# projection through the skinny small-N fp16 GEMV (`_sm70_ba_proj`) for
# decode-sized batches, and rearrange_mixed_qkv through the single-launch
# gdn_split3 kernel. Original: 1Cat-vLLM 1.2.2
...
def _skinny_split3_ok(x: torch.Tensor, wq: int, wk: int, wv: int) -> bool:
    try:
        from vllm.model_executor.kernels.linear.nvfp4.marlin import skinny_gdn_split3_ok
    except Exception:
        return False
    return skinny_gdn_split3_ok(x, wq, wk, wv)


def _sm70_ba_proj(layer, hidden_states: torch.Tensor) -> torch.Tensor:
    """in_proj_ba for decode-sized batches via the skinny small-N GEMV.

    The ba projection is a [M, 5120] x [5120, 48]^T fp16 GEMV per rank; cuBLAS
    runs it as a wmma tile kernel plus a split-K reduce (13.6 us per GDN
    layer). One block per output column with fp32 accumulation is ~3 us and
    within one fp16 ulp of cuBLAS. Falls back to the layer for anything else.
    """
    lin = layer.in_proj_ba
    w = getattr(lin, "weight", None)
    if (
        os.getenv("VLLM_SKINNY_BA_GEMV", "0") == "1"  # off by default: 1-ulp vs cuBLAS
        and w is not None
        ...
        and w.shape[0] <= 128
        and getattr(lin, "quant_method", None).__class__.__name__ == "UnquantizedLinearMethod"
    ):
        x = hidden_states if hidden_states.is_contiguous() else hidden_states.contiguous()
        out = torch.ops.skinny_nvfp4.gemv_f16(x, w)
        if getattr(lin, "gather_output", False) and lin.tp_size > 1:
            from vllm.distributed import tensor_model_parallel_all_gather

            out = tensor_model_parallel_all_gather(out)
        return out
    ba, _ = lin(hidden_states)
    return ba
...
class QwenGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def rearrange_mixed_qkv(self, mixed_qkv):
        ...
        # v100-skinny: one launch instead of three strided copies plus a cat.
        if seq_len <= 64 and _skinny_split3_ok(mixed_qkv, q_dim, k_dim, v_dim):
            q_c, k_c, v_c = torch.ops.skinny_nvfp4.gdn_split3(
                mixed_qkv, q_dim, k_dim, v_dim
            )
            return (
                q_c.view(1, seq_len, -1, self.head_k_dim),
                k_c.view(1, seq_len, -1, self.head_k_dim),
                v_c.view(1, seq_len, -1, self.head_v_dim),
            )
    ...
    def forward_hip(
        ...
        if GDN_AITER_TRITON_AVAILABLE:
            ...
            projected_states_ba = _sm70_ba_proj(self, hidden_states)
    ...
    def forward_cuda(
        ...
        ba = _sm70_ba_proj(self, hidden_states)
